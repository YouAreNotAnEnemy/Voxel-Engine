...
#ifdef LOGGING_DISABLE_VERBOSE
#define LOG_VERBOSE(...)
#else
#define LOG_VERBOSE(...) _logVerbose(__LINE__, __FILE__, __VA_ARGS__)
#endif

#ifdef LOGGING_DISABLE_DEBUG
#define LOG_DEBUG(...)
#else
#define LOG_DEBUG(...) _logDebug(__LINE__, __FILE__, __VA_ARGS__)
#endif
...
template<typename... Args>
inline void _logVerbose(size_t lineNumber, const char* file, const Args &... args) {
    std::string buf;
    variadicUnpack(buf, args...);
    logVerbose(lineNumber, file, buf.c_str(), buf.size());
}

// Faster shortcut if no variadic arguments are used
template<size_t N>
inline void _logVerbose(size_t lineNumber, const char* file, const char (& message)[N]) {
    //logInfo(0, 0, "Correct", 0);
    logVerbose(lineNumber, file, message, N-1);
}

template<typename... Args>
inline void _logDebug(size_t lineNumber, const char* file, const Args &... args) {
    std::string buf;
    variadicUnpack(buf, args...);
    logDebug(lineNumber, file, buf.c_str(), buf.size());
}

// Faster shortcut if no variadic arguments are used
template<size_t N>
inline void _logDebug(size_t lineNumber, const char* file, const char (& message)[N]) {
    //logInfo(0, 0, "Correct", 0);
    logDebug(lineNumber, file, message, N-1);
}